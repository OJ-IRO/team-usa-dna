// Milano-Cortina 2026 Winter Paralympics — Team USA medalists per sport.
// Same accuracy guardrails as the modern Paralympic enrichment.
//
// Output: data/winter-2026-paralympic.json.
...
const OUTPUT_PATH = path.join(ROOT, "data", "winter-2026-paralympic.json");
const EXISTING_FILES = [
  ...
  path.join(ROOT, "data", "paralympic-modern-extended.json"),
];
...
const YEAR = 2026;
const LABEL = "Milano-Cortina 2026 Winter Paralympics (held March 2026)";

const PARA_WINTER_SPORTS = [
  "Para Alpine Skiing",
  "Para Nordic Skiing",
  "Para Biathlon",
...
function slugify(name) {
  return "win26-para-" + name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
}
...
async function enumerateMedalists(sport) {
  const prompt = `You are an expert on Team USA Paralympic history with Google Search access. Find all Team USA athletes who won a medal in ${sport} at ${LABEL}.

REQUIREMENTS:
- Corroborate every entry against multiple authoritative sources (IPC paralympic.org, USOPC, Wikipedia, Team USA, NBC, OlymPedia, Sports Reference). If sources disagree by more than 3 cm on height or 3 kg on weight, return null. NEVER fabricate.
- Include ALL Team USA medalists at Milano-Cortina 2026 Winter Paralympics in this sport.
- Convert heights to integer cm, weights to integer kg.
- Age = ${YEAR} minus birth year.
- The "event" field should include the classification code if applicable (e.g. "Slalom Standing", "Sit-Ski Downhill", "Biathlon Visually Impaired").
...
Return ONLY a JSON array — no prose — each element:
{
  ...
  "age_at_year": <integer or null>,
  "events": [{ "year": ${YEAR}, "sport": "${sport}", "event": "<event w/ classification>", "medal": "Gold" | "Silver" | "Bronze" }],
  "medals": { "gold": <int>, "silver": <int>, "bronze": <int> },
  "_enrichmentNotes": "<source agreement / nulls>"
}

If no Team USA medalists in this sport, return [].`;
...
function normalize(raw) {
  ...
  return {
    ...
    _enrichmentNotes: typeof raw._enrichmentNotes === "string" ? raw._enrichmentNotes : "Milano-Cortina 2026 Winter Paralympic enrichment",
  };
}
...
async function main() {
  ...
  let lastReqAt = 0;
  for (const sport of PARA_WINTER_SPORTS) {
